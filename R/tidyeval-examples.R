...
summarize_var_new <- function(data, variable) {
	summarize(data,
			...
			p25   = quantile({{ variable }}, p = .25, na.rm = TRUE),
			p75   = quantile({{ variable }}, p = .75, na.rm = TRUE)
		)
}
...
summarize_two_vars <- function(data, variable1, variable2) {
	summarize (data,
						 cor_name = cor({{ variable1 }}, {{ variable2 }}, use = "complete.obs"),
						 cov_name = cov({{ variable1 }}, {{ variable2 }}, use = "complete.obs")
	)
}
